    ...
    with _action_lock:
        ...
        for i, a in enumerate(chunk):
            record = {
                ...
                "terminated" : len(chunk) == i + 1
            }
            _action_queues[start_expected_t + i].append(record)
            _created_counter += 1
            # while len(_new_action_queue) <= start_expected_t + i:
            #     _new_action_queue.append([])
            # _new_action_queue[start_expected_t + i].append(a)

        _schedule_cursor += _control_t
        #_control_t += len(chunk)


def pick_latest_for_expected_t(expected_timestep, created_timestep):
    """
    ...
    with _action_lock:
        ...
        if not q:
            return None
        # 最新创建在队尾
        if created_timestep == -1:
            return q[-1]
        else:
            for tmp_action in q:
                if tmp_action['created_timestep'] == created_timestep:
                    return tmp_action
