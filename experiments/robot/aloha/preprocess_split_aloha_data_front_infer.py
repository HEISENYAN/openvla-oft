...
def load_hdf5(demo_path):
    ...
    return success, action, image_dict
...
def load_and_preprocess_all_episodes(demo_paths, out_dataset_dir):
    ...
    """
    cam_names = ["image_front"]
    idx = 0
    for demo in tqdm(demo_paths):
        isSuccess, action, image_dict = load_hdf5(demo)
        # Save non-image info
        episode_len = image_dict["image_front"].shape[0]
        # Resize all images
...
def save_new_hdf5(out_dataset_dir, data_dict, episode_idx):
    ...
    H, W, C = data_dict["image_dict"]["image_front"][0].shape
    ...
    with h5py.File(
        ...
        #print(data_dict)
        episode_len = data_dict["isSuccess"].shape[0]
        ...
        _ = root.create_dataset("isSuccess", episode_len)
        root["/action"][...] = data_dict["action"]
        root["/isSuccess"][...] = data_dict["isSuccess"]
        ...
        _ = root.create_dataset("relative_action", (episode_len, 7))
