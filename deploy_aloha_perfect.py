...
def _enqueue_chunk_to_expected_queues(chunk, _schedule_cursor):
    ...
    _action_history[
        _schedule_cursor, _schedule_cursor : _schedule_cursor + len(chunk)
    ] = np.array(chunk)

    _action_infered_sign[_schedule_cursor] = 1

def temporal_ensembled_action(current_time):
    cur_actions = _action_history[:, current_time]
    indices = np.all(cur_actions != 0, axis=1)
    cur_actions = cur_actions[indices]
    exp_weights = np.exp(-0.01 * np.arange(len(cur_actions)))
    exp_weights = (exp_weights / exp_weights.sum())[:, None]
    sub_action = (cur_actions * exp_weights).sum(axis=0)

    return sub_action

_action_history = np.zeros([1500, 1500 + 8, 7])
_action_infered_sign = np.zeros(1500)
def clear_action_buffer():
    """episode 开始前清空所有队列与计数器。"""
    global _schedule_cursor, _created_counter, _control_t, _action_history, _action_infered_sign
    with _action_lock:
...
def _action_producer_loop(cfg: GenerateConfig, server_endpoint, resize_size, ros_operator):
    ...
    while not _action_stop_event.is_set() and not rospy.is_shutdown() and not stop_signal.is_set():
        ...
        # 入各 expected_t 队列
        _enqueue_chunk_to_expected_queues(actions_chunk, _schedule_cursor)
...
def run_openvla_oft(
    ...
    try:
        ...
        while t < cfg.max_steps and not rospy.is_shutdown():
            ...
            right_action = np.array(action).astype(float).tolist()
            # 如果返回包含双臂/夹爪等，可在此拆分：
    ...
    save = False if user_input.lower() == "n" else True
